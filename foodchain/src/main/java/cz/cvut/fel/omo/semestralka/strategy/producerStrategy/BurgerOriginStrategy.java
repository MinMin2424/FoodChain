...
public class BurgerOriginStrategy implements ProductOriginStrategy {
    @Override
    public List<ProductsCatalogue> getListProductOrigin() {
        List<ProductsCatalogue> origins = new ArrayList<>();
        origins.add(ProductsCatalogue.BUN);
        origins.add(ProductsCatalogue.BEEF);
        origins.add(ProductsCatalogue.TOMATO);
        return origins;
    }
}
